...
use objects::{Ctx, ID};
use plugins;
use plugins::debug::hider::Hider;
use plugins::debug::layers::ToggleableLayers;
use plugins::debug::steep::SteepnessVisualizer;
...
pub struct DebugMode {
    ...
    steepness: SteepnessVisualizer,

    // Ambient; they don't conflict with any of the main plugins.
    hider: Hider,
    pub layers: ToggleableLayers,
}
...
impl DebugMode {
    pub fn new(map: &Map) -> DebugMode {
        DebugMode {
            active_plugin: None,
            steepness: SteepnessVisualizer::new(map),
            hider: Hider::new(),
