...
fn workflow(mut wizard: WrappedWizard) -> Option<SpawnOverTime> {
    // This is kind of the ideal API -- it looks procedural.
    // Can implement by essentially trying to construct one of these every event(). The wizard
    // remembers intermediate state and immediately returns previously chosen answers when asked again.
    // It sort of mimics a coroutine in that sense.
    /*Some(SpawnOverTime {
    ...
    Some(SpawnOverTime {
        num1: wizard.input_usize("Question 1")?,
        num2: wizard.input_usize("Question 2")?,
        num3: wizard.input_usize("Question 3")?,
    })
}
