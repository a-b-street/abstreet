//! Loading large resources requires different strategies on native and web. Both cases are wrapped
//! up as a State that runs a callback when done.
...
mod native_loader {
    ...
    impl<A: 'static, T: 'static + Readable> FileLoader<A, T> {
        pub fn new_state(
            _: &mut EventCtx,
            path: String,
    ...
    impl<T: 'static + DeserializeOwned> Readable for T {
        fn read_file(path: String, timer: &mut Timer) -> Result<T> {
            // TODO We'll eventually want to remove the dependency on abstio
