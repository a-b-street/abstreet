...
fn test_map_importer() -> Result<()> {
    for name in [
        ...
        let path_types = abstio::path(format!("../tests/goldenfiles/turn_types/turn_types_{}.txt", map.get_name().map));
        // let mut f_types = File::create(&path_types)?;
        let turn_types = all_map_turns_as_string(&map);
        assert!(match compare_with_goldenfile(turn_types, path_types) {
            Ok(_) => true,
            Err(e) => {
                println!("Error in compare_with_goldenfile: {}", e);
                false
            },
        }
        ...
        let path_restrictions = abstio::path(format!("../tests/goldenfiles/turn_types/turn_restrictions_{}.txt", map.get_name().map));
        // let mut f_restrictions = File::create(&path_restrictions)?;
        let turn_restrictions = all_map_turn_restrictions_as_string(&map);
        assert!(match compare_with_goldenfile(turn_restrictions, path_restrictions) {
            Ok(_) => true,
            Err(_) => false,}
...
fn dump_turn_goldenfile(map: &Map) -> Result<()> {
    ...
    //     writeln!(f_types, "{} is a {:?}", t.id, t.turn_type)?;
    //     // writeln!(f, "{} is a {:?}", t.id, t.)?;
    // }
    let turn_types = all_map_turns_as_string(map);
    writeln!(f_types, "{}", turn_types)?;
    // compare_with_goldenfile(turn_types, path_types);
    ...
    //         writeln!(f_restrictions, "Turn from {} into {} is a {:?}, type {:?}, location {}", r1.id, r2, restriction, t_type, sign_pt)?;
    //     }
    // }
    let turn_restrictions = all_map_turn_restrictions_as_string(map);
    writeln!(f_restrictions, "{}", turn_restrictions)?;
    // compare_with_goldenfile(turn_restrictions, path_restrictions);

    Ok(())
}

fn all_map_turns_as_string(map: &Map) -> String {
...
fn all_map_turn_restrictions_as_string(map: &Map) -> String {
    let mut s = String::new();
    for r1 in map.all_roads() {
        for (restriction, r2) in &r1.turn_restrictions {
            let (t_type, sign_pt, r1_angle) = get_ban_turn_info(r1, map.get_r(*r2));
...
fn compare_with_goldenfile(actual: String, goldenfile_path: String) -> Result<()> {
    ...
    let binding = String::from_utf8(abstio::slurp_file(&goldenfile_path)?).unwrap().clone();
